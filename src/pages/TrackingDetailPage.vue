...
const obligationsForPeriod = computed(() => {
  ...
  return obligationsStore.obligations.filter(o =>
    !o.trackingEntryId || o.trackingEntryId === entry.value!.id
  )
})
...
async function markObligationDone(obligation: Obligation) {
  ...
  if (effects.length > 0) {
    await transactionsStore.create({
      ...
      date: isoDate(),
      time: isoTime(),
      description: `${obligation.name} - ${t('tracking.obligationCompleted')}`,
      effects,
      ...
      <ClayCard>
        <div class="flex items-center justify-between">
          ...
          <div class="text-center">
            <p class="text-xs text-clay-muted">{{ t('tracking.currentPeriod') }}</p>
          ...
          <ClayCard v-for="obl in incomeObligations" :key="obl.id" padding="p-3">
            <div class="flex items-center justify-between">
              ...
              <ClayBadge v-else variant="income">
                {{ t('tracking.obligationCompleted') }}
              </ClayBadge>
            </div>
          </ClayCard>
        </div>
